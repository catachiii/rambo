...
class DelayedDCMotor(DCMotor):
    ...
    def __init__(
        ...
        # instantiate the delay buffers
        self.positions_delay_buffer = DelayBuffer(cfg.max_num_time_lags, self._num_envs, device=device)
        self.velocities_delay_buffer = DelayBuffer(cfg.max_num_time_lags, self._num_envs, device=device)
        self.efforts_delay_buffer = DelayBuffer(cfg.max_num_time_lags, self._num_envs, device=device)
        # all of the envs
    ...
    def reset(self, env_ids: Sequence[int]):
        ...
        if self.cfg.num_time_lags is None:
            # set a new random delay for environments in env_ids
            ...
            time_lags = self.positions_delay_buffer.time_lags
            time_lags[env_ids] = self.cfg.num_time_lags
